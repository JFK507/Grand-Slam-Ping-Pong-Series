/* El árbol entero: dos ramas de cuartos, sus semis y la final. */
import React from 'react';
import { C } from '../lib/constantes.js';
import { contarSets, tieneDetalleSets } from '../lib/reglas.js';
import { Baja, Card, Eyebrow, Vacio } from '../ui/primitivas.jsx';
import { Avatar } from '../ui/fotos.jsx';

export default function CuadroCompleto({ t, name }) {
  if (!t.qf) return <Vacio>El cuadro aparece cuando se completen los 8 clasificados.</Vacio>;

  const seed = (id) => {
    const i = t.qualified.indexOf(id);
    return i >= 0 ? `A${i + 1}` : '';
  };
  ...
  const Duelo = ({ et, a, b, sa, sb, winner, listo, tone }) => (
    <Card style={{ padding: '9px 11px', borderColor: tone || C.line }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span className="num" style={{ fontSize: 10, fontWeight: 800, color: tone || C.dim, width: 20 }}>
          {et}
        </span>
          ...
          {[[a, sa], [b, sb]].map(([id, s], i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                ...
                <span style={{
                  flex: 1, fontSize: 12.5, fontWeight: gana ? 700 : 400,
                  color: gana ? C.gold : C.chalk,
                  whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                }}>
                  {name(id)}
                </span>
                {seed(id) && (
                  <span className="num" style={{ fontSize: 10, color: C.dim }}>{seed(id)}</span>
                )}
                <span className="num" style={{
                  fontSize: 15, fontWeight: 800, width: 20, textAlign: 'right',
                  color: gana ? C.gold : C.dim,
                }}>
                  {listo || sa || sb ? s : '·'}
                </span>
              </div>
  ...
  const PorJugar = ({ et, texto }) => (
    <Card style={{
      padding: '13px 11px', borderStyle: 'dashed',
  ...
  const f = t.final;
  const wa = f ? contarSets(f, 'a') : 0;
  const wb = f ? contarSets(f, 'b') : 0;

  return (
    <div style={{ padding: 16, display: 'grid', gap: 14 }}>
      {[0, 1].map((r) => {
        const m1 = t.qf[r === 0 ? 0 : 1];
        const m2 = t.qf[r === 0 ? 2 : 3];
        const s = t.sf ? t.sf[r] : null;
        return (
          <div key={r} style={{ display: 'grid', gap: 6 }}>
            <Eyebrow color={C.red}>Rama {r + 1}</Eyebrow>
            <Duelo et={`C${m1.n}`} a={m1.a} b={m1.b} sa={m1.sa} sb={m1.sb}
              winner={m1.winner} listo={m1.locked} />
            <Duelo et={`C${m2.n}`} a={m2.a} b={m2.b} sa={m2.sa} sb={m2.sb}
              winner={m2.winner} listo={m2.locked} />
            <Baja />
            {s
              ? <Duelo et={`S${s.n}`} a={s.a} b={s.b} sa={s.sa} sb={s.sb}
                  winner={s.winner} listo={s.locked} tone={C.gold} />
      ...
      {f ? (
        <Card style={{ borderColor: C.gold, background: f.locked ? C.goldInk : C.card }}>
          {[[f.a, wa], [f.b, wb]].map(([id, w], i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0' }}>
                ...
                <span style={{
                  flex: 1, fontSize: 14, fontWeight: gana ? 700 : 400,
                  color: gana ? C.gold : C.chalk,
                }}>
                  {name(id)}
                </span>
                <span className="num" style={{
                  fontSize: 22, fontWeight: 800, color: gana ? C.gold : C.dim,
                }}>
                  {w}
                </span>
              </div>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 9, letterSpacing: '.16em', color: C.dim, fontWeight: 700 }}>
              SETS
            </span>
            {tieneDetalleSets(f)
              ? f.sets.map((s, i) => (
                <span key={i} className="num" style={{ fontSize: 13, color: C.dim }}>{s.a}-{s.b}</span>
              ))
              : <span style={{ fontSize: 11, color: C.dim }}>marcador por set no registrado</span>}
          </div>
        </Card>
