...
export default function Ronda({ t, name, update, fase }) {
  ...
  const setMatch = (i, patch, now = false) => update({
    [fase]: list.map((m, j) => (j === i ? { ...m, ...patch } : m)),
  }, now);

  const todos = list.length > 0 && list.every((m) => m.locked);

  /* X1 vs X3 y X2 vs X4, como manda el formato. */
  const avanzar = () => {
    ...
    if (fase === 'qf') {
      ...
      update({
        final: { a: w[0], b: w[1], sets: [{ a: 0, b: 0 }], winner: null, locked: false },
        stage: 'final',
      }, true);
    }
  };
    ...
    return (
      <Marcador
        ...
        onPoint={(side, d) => {
          ...
          setMatch(open, {
            sa: na, sb: nb,
            winner: isWin(na, nb, target, false) ? m.a
              : isWin(nb, na, target, false) ? m.b : null,
          });
        }}
  ...
  return (
    <div style={{ padding: 16, display: 'grid', gap: 10 }}>
      ...
      {list.map((m, i) => (
        <Card key={i} onClick={() => setOpen(i)}
          style={{ cursor: 'pointer', borderColor: m.locked ? C.line : C.red }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span className="num" style={{
              fontSize: 11, color: m.locked ? C.dim : C.red, fontWeight: 800, width: 20,
            }}>
              {pref}{m.n}
            </span>
      ...
      {todos && (
        <Btn tone="gold" full onClick={avanzar}>
          {fase === 'qf' ? 'Ir a semifinales' : 'Ir a la final'}
        </Btn>
      )}
    </div>
  );
