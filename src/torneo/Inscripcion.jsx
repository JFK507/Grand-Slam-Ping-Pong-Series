/* Etapa 1: inscribir jugadores y sortear los lados de la mesa. */
  ...
  const addNuevo = () => {
    ...
    commit({
      ...
      tournaments: db.tournaments.map((x) =>
        (x.id === t.id ? { ...x, entrants: [...x.entrants, id] } : x)),
    }, true);
    setVal('');
  };

  const toggle = (id) => update({
    entrants: t.entrants.includes(id)
      ? t.entrants.filter((x) => x !== id)
      : [...t.entrants, id],
  }, true);

  /* Mitades iguales; si son impares, la izquierda se queda con el extra. */
  const sortear = () => {
    ...
    <div style={{ padding: 16, display: 'grid', gap: 14 }}>
      <div style={{ display: 'flex', gap: 8 }}>
        <input value={val} onChange={(e) => setVal(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && addNuevo()}
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8 }}>
          ...
          <div className="num" style={{ fontSize: 20, fontWeight: 800, color: n >= 8 ? C.gold : C.red }}>
            {n}
          </div>
          {n < 8 && (
            <div style={{ fontSize: 11, color: C.dim }}>faltan {8 - n} para poder clasificar 8</div>
          )}
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {t.entrants.map((id) => (
            <Chip key={id} onClick={() => toggle(id)} tone="on" id={id} name={nm(id)}>
              {nm(id)} <span style={{ opacity: 0.5, marginLeft: 3 }}>×</span>
            </Chip>
          ))}
          {n === 0 && (
            <div style={{ fontSize: 12, color: C.dim }}>
              Escribe un nombre y toca + para inscribir.
            </div>
          )}
        </div>
      </div>
      ...
      {disp.length > 0 && (
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
            {disp.map((p) => (
              <Chip key={p.id} onClick={() => toggle(p.id)} id={p.id} name={p.name}>{p.name}</Chip>
            ))}
          </div>
        </div>
      )}
