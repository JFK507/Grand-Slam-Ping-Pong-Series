/* Marcador de dos paneles grandes: se toca el lado del que anota. */
...
export default function Marcador({
  titulo, idA, idB, nameA, nameB, a, b, target, adv,
  onPoint, winner, winnerId, onConfirm, onBack, extra,
}) {
  ...
  const Side = ({ id, n, v, side, lead }) => (
    <div onClick={() => tocar(side, 1)}
      style={{
        background: lead ? C.goldInk : C.card,
        border: `1px solid ${lead ? C.gold : C.line}`,
        borderRadius: 14, padding: '14px 10px 12px', textAlign: 'center', cursor: 'pointer',
        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
        minHeight: 205, justifyContent: 'center',
      }}>
      <Avatar id={id} name={n} size={44} ring={lead ? C.gold : C.line} />
      <div style={{
        fontSize: 12.5, fontWeight: 600,
        color: lead ? C.gold : C.chalk, wordBreak: 'break-word',
      }}>
        {n}
      </div>
      <div className="num" style={{
        fontSize: 60, fontWeight: 800, lineHeight: 0.9,
        color: lead ? C.gold : C.chalk,
      }}>
        {v}
      </div>
      <button onClick={(e) => { e.stopPropagation(); tocar(side, -1); }}
        style={{
          ...
          borderRadius: 8, padding: '5px 16px', fontSize: 13, cursor: 'pointer',
        }}>
        −1
      </button>
    </div>
    ...
    <div style={{ padding: 16, display: 'grid', gap: 12 }}>
      ...
      {winner ? (
        ...
        <div style={{ fontSize: 11, color: C.dim, textAlign: 'center' }}>
          Toca el lado del jugador que anota.
        </div>
      )}
    </div>
  );
