/* Etapa 3: se toca el nombre del que anota. Los primeros 8 en llegar a 7 clasifican. */
import React from 'react';
import { C } from '../lib/constantes.js';
import { shuffle, vibra } from '../lib/util.js';
import { usePantallaViva } from '../ui/usePantallaViva.js';
import { Btn, Card, Eyebrow, Pips } from '../ui/primitivas.jsx';
import { Avatar } from '../ui/fotos.jsx';

export default function Clasificacion({ t, name, update }) {
  usePantallaViva(true);
  const { izq, der } = t.groups;
  const clas = t.clas;
  const qual = t.qualified;
  ...
  const add = (id, d) => {
    ...
    const nc = { ...clas, [id]: v };
    let nq = [...qual];
    if (v >= 7 && !nq.includes(id) && nq.length < 8) {
      nq.push(id);
      vibra([28, 60, 28, 60, 90]);
    } else if (d > 0) {
      vibra(12);
    }
  ...
  /* El cuadro se sortea solo, al azar. */
  const armarCuadro = () => {
    const idx = shuffle([0, 1, 2, 3, 4, 5, 6, 7]);
    update({
      qf: [0, 1, 2, 3].map((i) => ({
        n: i + 1,
        a: qual[idx[i * 2]], b: qual[idx[i * 2 + 1]],
      ...
      {ids.map((id) => {
        ...
        return (
          <div key={id} onClick={() => add(id, 1)}
            style={{
              background: ok ? C.goldInk : C.card,
              border: `1px solid ${ok ? C.gold : C.line}`,
              borderRadius: 11, padding: '8px 9px', cursor: ok ? 'default' : 'pointer',
            }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {ok
                ? <div className="num" style={{ fontSize: 15, fontWeight: 800, color: C.gold }}>A{seed + 1}</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
              ...
              {!ok && v > 0 && (
                <button onClick={(e) => { e.stopPropagation(); add(id, -1); }}
                  style={{
                    ...
                    color: C.dim, borderRadius: 6, width: 24, height: 20,
              ...
              {ok && (
                <div style={{
                  marginLeft: 'auto', fontSize: 8.5, letterSpacing: '.16em',
  ...
  return (
    <div style={{ padding: 16, display: 'grid', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <div className="num" style={{ fontSize: 26, fontWeight: 800, color: listo ? C.gold : C.chalk }}>
          {qual.length}
        </div>
      ...
      {listo && (
        <Card style={{ background: C.goldInk, borderColor: C.gold, textAlign: 'center' }}>
          <div className="num" style={{ fontSize: 19, fontWeight: 800, color: C.gold, marginBottom: 10 }}>
