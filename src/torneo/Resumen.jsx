...
import { calcTorneo, etiquetaPts, ordenRanking } from '../lib/reglas.js';
      ...
      <div style={{ display: 'grid', gap: 5 }}>
        {filas.map((r, i) => (
          <div key={r.id} style={{
            ...
            border: `1px solid ${r.pts >= 25 ? C.gold : C.line}`,
            ...
            <span className="num" style={{
              width: 44, textAlign: 'right', fontSize: 16, fontWeight: 800,
              color: r.pts ? C.gold : C.dim,
            }}>
              {r.pts}
            </span>
            <span className="num" style={{ width: 44, textAlign: 'right', fontSize: 14, color: C.dim }}>
              {r.dif > 0 ? '+' : ''}{r.dif}
            </span>
          </div>
        ))}
